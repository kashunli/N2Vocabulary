...
├── jlpt-audio-cutter/           ← Codex skill for audio clip operations
...
| `WHISPER_TRANSCRIBE_BACKEND` | openai | `whisper_cpp` for local Windows GPU (AMD RX 6950 XT) |
...
- The Windows GPU backend uses `WHISPER_TRANSCRIBE_BACKEND=whisper_cpp` with `tools/whispercpp-windows/whisper-cli.exe`
- Read `RESUME.md` for full project history and design decisions
- The `jlpt-audio-cutter/` directory contains a Codex skill — read `SKILL.md` for audio operations
