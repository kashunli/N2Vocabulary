...
impl Options {
    fn from_env() -> Result<Self> {
        let mut db_path = PathBuf::from("../../output/n2vocab.sqlite");
        let mut apply = false;
...
fn print_help() {
    println!(
        "Clean entry_examples.text for natural sentence TTS.\n\
         \n\
         Usage:\n\
           cargo run --bin clean_sentence_text -- [--db ../../output/n2vocab.sqlite] [--apply]\n\
         \n\
         ...
         from position 0, and clears generated Edge TTS links for rows whose\n\
         text changed."
    );
}
