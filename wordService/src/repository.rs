...
fn ensure_silence_clip(path: &Path, seconds: u64) -> Result<()> {
    if path.is_file() {
        return Ok(());
    }
    let parent = path
...
fn write_flagged_audio_concat_list<F>(
    path: &Path,
...
fn run_ffmpeg<I, S>(args: I) -> Result<()>
where
...
fn generated_word_clip_path(generated_dir: &str, entry_id: i64) -> Result<String> {
    let dir = normalize_generated_audio_dir(generated_dir)?;
    Ok(format!("{dir}/word{entry_id}.mp3"))
}

fn word_text_for_tts(word: &str, reading: Option<&str>) -> String {
