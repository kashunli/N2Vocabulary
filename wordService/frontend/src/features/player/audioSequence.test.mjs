...
test("the default recipe keeps the current word-then-sentence behavior", () => {
  ...
  assert.deepEqual(materializeAudioSequence(sequence, "both", entry).map((step) => step.occurrenceId), ["word-1:0", "sentence-1:0"]);
});
...
test("a repeated recipe step becomes separate playback occurrences", () => {
  ...
  const occurrences = materializeAudioSequence(sequence, "both", entry);
...
test("the recipe skips unavailable audio and supports a mode filter", () => {
  const sequence = normalizeAudioSequence({
    steps: [
  ...
  assert.deepEqual(materializeAudioSequence(sequence, "words", {word_audio_url: "/audio/word.mp3"}).map((step) => step.element), ["word", "word"]);
  assert.deepEqual(materializeAudioSequence(sequence, "both", {word_audio_url: "/audio/word.mp3"}).map((step) => step.element), ["word", "word"]);
});
