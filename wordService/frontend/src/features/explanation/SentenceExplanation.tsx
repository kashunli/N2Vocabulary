...
export function SentenceExplanation({sentenceTranslations = [], value}: SentenceExplanationProps) {
  const {copy} = useI18n();
  return (
    <section className="react-sentence-explanation" aria-label={copy.sentenceExplanation}>
      <MarkdownContent value={value} omitParagraphs={sentenceTranslations} />
    </section>
  );
}
